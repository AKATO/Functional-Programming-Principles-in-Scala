...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.groupBy((elem:Char)=>elem.toLower).map((mapping)=>(mapping._1, mapping._2.length)).toList.sorted
  ...
  def wordAnagrams(word: Word): List[Word] = occWords(wordOccurrences(word))
